...
// 1appstack data schema here: https://docs.google.com/spreadsheets/d/1mLD1MeFxumHrUNGBDzHVLbCk0dvyoOUIztV7mpCb9vY/edit?usp=sharing

import 'dart:typed_data';

import 'package:one_app_stack_storage_api/one_app_stack_storage_api.dart';

import 'firebase_otherapp_helper.dart';
...
class UserStorage implements AbstractUserStorage {
  ...
  Future<UserInfo?> loadUserInfo(String uid) async {
    final snapshot = await firebaseHelper.userDocumentReference(uid).once();
...
class ProjectStorage implements AbstractProjectStorage {
  ...
  Future<void> checkInvites(String email) async {
    ...
    await message.call({'email': email, 'cleanEmail': cleanEmail});
  }
  ...
  Future<List<Map<String, dynamic>>> loadProjectAuth(String projectId) async {
    final snapshot =
        await firebaseHelper.projectAuthReference(projectId).once();
    final map = Map<String, dynamic>.from(snapshot.value);
    final secondaryMap =
        map.values.map((e) => Map<String, dynamic>.from(e)).toList();
    return secondaryMap;
  ...
  Future<List<DocumentInfo>> loadDocumentDefinitions(String projectId) async {
    final snapshot = await firebaseHelper
  ...
  Future<ProjectInfo> loadProjectInfo(String projectId) async {
    final snapshot =
        await firebaseHelper.projectInfoReference(projectId).once();
    final map = Map<String, dynamic>.from(snapshot.value);
...
class AppStorage implements AbstractAppStorage {
  ...
  Future<Map<String, dynamic>> _realtimeDocuments(List<String> path) async {
    final snapshot = await otherAppHelper.realtimeReferenceByPath(path).once();
    if (snapshot.value == null) {
      return {};
    }
    Map<String, dynamic> documentsMap = snapshot.value;
    return documentsMap;
  }
  ...
  Future<Uri> getAppFileUri(List<String> path, String filename) async {
    final urlString =
        await otherAppHelper.fileStorageByPath(path, filename).getDownloadURL();
...
//   FirebaseFirestore firestoreSecondary = FirebaseFirestore.instanceFor(
//       app: Firebase.app(
//           'SecondaryApp')); /*https://firebase.google.com/docs/projects/multiprojects */
}
